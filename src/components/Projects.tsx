...
export default function Projects() {
  ...
  return (
    <Section id="projects" title="Featured Work" eyebrow="Personal Projects" className="py-8">
            ...
            >
              <div className="flex flex-1 flex-col p-3">
                <div className="mb-1 flex items-center justify-between">
                  <h3 className="text-sm font-semibold text-text-h">{project.name}</h3>
                  <a
                    href={project.link}
                    target="_blank"
                    rel="noreferrer"
                    aria-label={`${project.name} on GitHub`}
                    className="text-[11px] text-text-dim transition-colors"
                ...
                <p className="mb-1.5 text-xs leading-snug text-text">{project.description}</p>
                  ...
                  <div className="mt-1.5 flex flex-wrap gap-1.5">
                    {project.images.map((img) => {
                      ...
                      return (
                        ...
                        >
                          <img src={src} alt={`${project.name} screenshot`} className="h-9 w-12 object-cover" />
                        </button>
                      )
                    })}
                  </div>
                ...
                {project.demo && (
                  <button
                    ...
                    className="mt-1.5 flex items-center justify-center gap-2 rounded-lg px-3 py-1 text-xs font-semibold text-black transition-transform hover:scale-[1.02]"
      ...
      <p className="mt-3 text-center text-[11px] text-text-dim">
        Live demos run on Groq's free-tier Llama API to keep hosting costs down — every backend here can be
        reconfigured to run any model on any infrastructure.
      </p>
